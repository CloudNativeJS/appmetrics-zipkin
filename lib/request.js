...
function Request( type, name, root , traceId) {
    this.children = [];
    this.name = name;
    this.type = type;

    if (root === true) {
      // console.log("[Request] I am root");
    ...
    ++nextEventId;
    if( this.parent ) {
        // console.log("[Request] I am not root");
...
Request.prototype.start = function(traceId) {
    this.active = true;
    if( !this.timer ) {
...
exports.startRequest = function( type, name, root, eventTimer, traceId ) {
    var req = new Request( type, name, root, traceId );
    if( eventTimer ) {
